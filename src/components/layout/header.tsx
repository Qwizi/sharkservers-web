...
const Header = () => {
      ...
      case MenuTypeEnum.NORMAL:
        return menuLinks.map((link, i) => (
          <Link
            key={i}
            href={link.path}
            className="text-sm leading-6 text-slate-200 block rounded-md px-3 py-2 font-medium hover:bg-slate-800"
  ...
  return (
    <header className="border-b">
      <nav
        className="mx-auto flex container items-center justify-between p-6 "
        aria-label="Global"
      >
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end gap-x-12">
          {status == "authenticated" && !isVip && (
            <Button
              ...
              size="sm"
              className="mr-2"
              onClick={vipModal.onOpen}
            >
              Ulepsz konto
            </Button>
          )}
          {status == "unauthenticated" && (
            <MenuLinks type={MenuTypeEnum.AUTH} />
          )}
