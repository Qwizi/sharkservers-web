...
export default function ThreadBadges({
  categoryName,
  is_closed,
  is_pinned,
  serverName,
  ...
  const PinnedStatusBadge = () => {
    if (!is_pinned) return;
    return <Badge className="bg-yellow-400 text-white">Wątek przypiety</Badge>;
  };

  const CategoryNameBadge = () => {
    return <Badge className="text-white">{categoryName}</Badge>;
  };
