...
import { CreateApplicationThreadFormSchema, CreateApplicationThreadFormSchemaInputs } from "@/schemas"
...
export default function CreateApplicationThreadForm({ category, servers }: CreateApplicationThreadFormProps) {
...
`
    ...
    useEffect(() => {
        ...
        if (user) {
            ...
            if (user?.player) {
                replacedContent = replacedContent.replace("{steamid32}", user.player.steamid32)
            }
        }

        if (serverIdWatch) {
            replacedContent = replacedContent.replace("{serverName}", servers?.items.find((server) => server.id === Number(serverIdWatch))?.name || "")
        }
        ...
        if (experienceWatch) {
            replacedContent = replacedContent.replace("{experience}", experienceWatch)
        }
    ...
    }, [category, user, serverIdWatch, ageWatch, experienceWatch, reasonWatch, servers])
    ...
    async function onSubmit(data: CreateApplicationThreadFormSchemaInputs) {
        ...
        if (response.serverError) {
            ...
            if (response?.data)  {
                form.reset();
                router.push(`/forum/${slugify(response?.data?.title)}-${response?.data?.id}`)
                toast({
                    variant: "success",
                    title: "Sukces!",
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <MarkdownEditor {...field} height={"500px"} enablePreview={false} readOnly={true}/>
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                    )}
