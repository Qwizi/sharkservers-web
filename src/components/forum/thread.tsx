...
export default function Thread({ ...props }: ThreadOut) {
  ...
  return (
    <Card
      className={
        is_pinned ? "border border-blue-400 p-4 animate-pulse" : "p-4 h-[250px]"
      }
    >
      <div className="flex">
        ...
        <div className="flex flex-col ml-5">
          ...
          <span className="text-slate-500">
            {dateTimeFormatter.format(new Date(created_at))}
          </span>
        </div>
      </div>
      <div>
        <h4 className="text-2xl p-2 mt-2">
          <Link href={`/forum/${slugify(title)}-${id}`}>{title}</Link>
        </h4>
      </div>
      <div className="flex flex-col mt-10 p-4">
        <Separator />
        <div>Postów: {post_count}</div>
        <div>
          <ThreadBadges
