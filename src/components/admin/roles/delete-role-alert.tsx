...
export default function DeleteUserAlert({ ...props }: DeleteUserAlertProps) {
    ...
    return (
        <AlertDialog open={props.isDeleteModalOpen} onOpenChange={props.setIsDeleteModalOpen}>
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle>Czy napewno chcesz usunąc role {props.name} ({props.id})?</AlertDialogTitle>
                    <AlertDialogDescription>
                        Ta akcja jest nieodwracalna. Spowoduje trwałe usunięcie roli {props.name} ({props.id}) z bazy danych.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
