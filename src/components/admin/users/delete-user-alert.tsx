...
export default function DeleteUserAlert({ ...props }: DeleteUserAlertProps) {
    ...
    return (
        <AlertDialog open={props.isDeleteModalOpen} onOpenChange={props.setIsDeleteModalOpen}>
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle>Czy napewno chcesz usunąc użytkownika {props.username} ({props.id})?</AlertDialogTitle>
                    <AlertDialogDescription>
                        Ta akcja jest nieodwracalna. Spowoduje trwałe usunięcie konta {props.username} ({props.id}) z bazy danych.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
