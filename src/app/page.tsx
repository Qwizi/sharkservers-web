...
async function fetchData(api: ApiClient) {
  ...
  const servers =
    serversResult.status === "rejected" ? null : serversResult.value;
  const categories =
    categoriesResult.status === "rejected" ? null : categoriesResult.value;
  const threads =
    threadsResult.status === "rejected" ? null : threadsResult.value;
  const lastThreads =
    lastThreadsResult.status === "rejected" ? null : lastThreadsResult.value;
  ...
  const lastOnlineUsers =
    lastOnlineUsersResultsResult.status === "rejected"
      ? null
      : lastOnlineUsersResultsResult.value;
  const lastPlayers =
    lastPlayersResult.status === "rejected" ? null : lastPlayersResult.value;
  return {
    servers,
    categories,
    threads,
...
export default async function Home() {
  ...
  return (
    <>
      {servers && <ServersTable data={...servers} />}

      <Chat />
      {lastPlayers && <LastPlayers players_data={lastPlayers} />}
      <ForumContainer
      ...
      {lastOnlineUsers && <LastOnlineUsers {...lastOnlineUsers} />}
      <WebsiteStats
        users_total={users ? users.total : 0}
        threads_total={threads ? threads.total : 0}
        posts_total={posts ? posts.total : 0}
        last_user={users ? users.items[0] : null}
      />
    </>
  );
}
